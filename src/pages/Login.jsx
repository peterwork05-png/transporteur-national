...
// PINs - in production these would be in a database
const ADMIN_PIN = '1234';
const DRIVERS = [
  { id: 'marc',    name: 'Marc Dumont',      role: 'local',   pin: '1111', initials: 'MD', color: '#C0392B',  path: '/driver/local/marc' },
  { id: 'peter',   name: 'Peter',            role: 'local',   pin: '2222', initials: 'PE', color: '#7C3AED',  path: '/driver/local/peter' },
  { id: 'jeanluc', name: 'Jean-Luc Bergeron',role: 'ontario', pin: '3333', initials: 'JL', color: '#8B4513',  path: '/driver/ontario' },
  { id: 'pierre',  name: 'Pierre Tremblay',  role: 'quebec',  pin: '4444', initials: 'PT', color: '#0F6E56',  path: '/driver/quebec' },
];

const PinPad = ({ title, subtitle, onSuccess, onBack, correctPin }) => {
  ...
  return (
    <div className="w-full max-w-xs mx-auto">
      <button onClick={onBack} className="flex items-center gap-2 text-sm mb-6" style={{color:'rgba(250,247,240,0.4)'}}>
        ← Back
      </button>
...
export default function Login() {
  ...
  const [screen, setScreen] = useState('home'); // home | admin-pin | drivers | driver-pin
  const [selectedDriver, setSelectedDriver] = useState(null);

  const enterAdmin = () => { login('admin', 'Admin'); navigate('/admin'); };
  const enterDriver = (driver) => { login(driver.id, driver.name); navigate(driver.path); };
