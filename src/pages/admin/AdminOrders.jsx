...
const STATUS_STEPS  = ['waiting','accepted','picked','enroute','delivered'];
const STATUS_LABELS = { waiting:'Order placed', accepted:'Accepted', picked:'Picked up', enroute:'En route', delivered:'Delivered', attempted:'Attempted delivery' };
...
export default function AdminOrders() {
  ...
  const filtered = displayOrders.filter(o => {
    ...
    if (tab === 'Active')     return ['waiting','accepted','picked','enroute'].includes(o.status);
  ...
  const TABS = [
    ...
    { label:`Active (${displayOrders.filter(o=>['waiting','accepted','picked','enroute'].includes(o.status)).length})`, val:'Active' },
  ...
  const markDelivered = async () => {
    ...
    await fetch(`/api/orders/${selected.id}/status`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'delivered', delivered_at: now, recipient_name: 'Admin' }),
    });
    if (period === 'today') await fetchOrders();
    else setAllOrders(prev => prev.map(o => o.id === selected.id ? { ...o, status: 'delivered', delivered_at: now } : o));
  ...
  const markAttempted = async () => {
    ...
    if (note) {
      await fetch(`/api/orders/${selected.id}/edit`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ notes: `[Attempted delivery] ${note}` }),
      });
    }
    if (period === 'today') await fetchOrders();
    else setAllOrders(prev => prev.map(o => o.id === selected.id ? { ...o, status: 'attempted' } : o));
  ...
  const reschedule = async () => {
    ...
    if (period === 'today') await fetchOrders();
    else setAllOrders(prev => prev.map(o => o.id === selected.id ? { ...o, status: 'waiting' } : o));
    ...
    <div className="p-4 md:p-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{color:'var(--tn-gold)'}}>
            {displayOrders.length} orders {period === 'today' ? 'today' : period === '7days' ? 'in last 7 days' : 'all time'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="badge badge-info">
            {displayOrders.filter(o=>['waiting','accepted','picked','enroute'].includes(o.status)&&(o.driver||o.driver_id)).length} active
          </span>
        </div>
      </div>
      ...
      {period === 'all' && (
        <div className="flex gap-2 mb-4 items-center flex-wrap">
          <div><label className="label">From</label><input type="date" className="input text-sm" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={{width:'140px'}}/></div>
          <div><label className="label">To</label><input type="date" className="input text-sm" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={{width:'140px'}}/></div>
          {(dateFrom || dateTo) && <button onClick={()=>{setDateFrom('');setDateTo('');}} className="btn btn-outline btn-sm mt-4">Clear</button>}
        </div>
      )}
                ...
                <div className="space-y-2">
                  {selected.photo_url && (
                    <div>
                      ...
                      <img src={selected.photo_url} alt="Delivery proof" className="w-full rounded-xl object-cover" style={{maxHeight:'200px'}}/>
                    </div>
                  )}
                  {selected.signature_url && (
                    <div>
                      <p className="text-xs mb-1 font-medium" style={{color:'var(--tn-gold)'}}>✍️ Signature — {selected.recipient_name}</p>
                      <img src={selected.signature_url} alt="Signature" className="w-full rounded-xl" style={{maxHeight:'120px', background:'white', padding:'8px', border:'0.5px solid var(--tn-border)'}}/>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4" style={{background:'rgba(26,18,8,0.7)'}}>
          <div className="rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" style={{background:'var(--tn-cream)'}}>
            ...
            <div className="p-5 space-y-3">
              <div><label className="label">Delivery address</label><input className="input" value={editForm.address} onChange={e=>setEditForm(f=>({...f,address:e.target.value}))} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Boxes</label><input type="number" className="input" value={editForm.boxes} onChange={e=>setEditForm(f=>({...f,boxes:parseInt(e.target.value)||1}))} /></div>
              ...
              <div><label className="label">Date</label><input type="date" className="input" value={editForm.date} onChange={e=>setEditForm(f=>({...f,date:e.target.value}))} /></div>
              <div>
              ...
              <div><label className="label">Notes</label><textarea className="input" rows={3} style={{resize:'none'}} value={editForm.notes} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))} /></div>
      ...
      {showCreate && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{background:'rgba(26,18,8,0.7)'}}>
          <div className="rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" style={{background:'var(--tn-cream)'}}>
            ...
            <div className="p-5 space-y-3">
              <div><label className="label">Order ID (optional)</label><input className="input font-mono" placeholder="e.g. DEL-2026-9999" value={createForm.id} onChange={e=>setCreateForm(f=>({...f,id:e.target.value}))} /></div>
              <div><label className="label">Delivery address *</label><input className="input" placeholder="123 Rue Example, Montréal" value={createForm.address} onChange={e=>setCreateForm(f=>({...f,address:e.target.value}))} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Boxes</label><input type="number" className="input" value={createForm.boxes} onChange={e=>setCreateForm(f=>({...f,boxes:parseInt(e.target.value)||1}))} /></div>
                <div><label className="label">Amount ($)</label><input type="number" step="0.01" className="input" placeholder="0.00" value={createForm.amount} onChange={e=>setCreateForm(f=>({...f,amount:e.target.value}))} /></div>
              </div>
              <div><label className="label">Date</label><input type="date" className="input" value={createForm.date} onChange={e=>setCreateForm(f=>({...f,date:e.target.value}))} /></div>
              <div>
                <label className="label">Assign driver (optional)</label>
                <select className="input" value={createForm.driver_id} onChange={e=>setCreateForm(f=>({...f,driver_id:e.target.value}))}>
                  <option value="">— Unassigned —</option>
                  {drivers.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
              </div>
              <div><label className="label">Notes (optional)</label><textarea className="input" rows={3} style={{resize:'none'}} placeholder="Delivery notes..." value={createForm.notes} onChange={e=>setCreateForm(f=>({...f,notes:e.target.value}))} /></div>
              <div className="flex gap-2 pt-2">
